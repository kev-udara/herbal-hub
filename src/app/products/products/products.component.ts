...
export class ProductsComponent implements OnInit, OnDestroy {
  ...
  private updateComponentHeight() {
    ...
    let newHeight = 145.5; // Default height

    if (this.itemsPerPage === 6) {
      newHeight *= 1.55; // Apply the multiplication factor only when itemsPerPage is 9
    }

    if (this.itemsPerPage === 9) {
      newHeight *= 2.17; // Apply the multiplication factor only when itemsPerPage is 9
    }
